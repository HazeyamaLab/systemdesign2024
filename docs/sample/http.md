...
## JavaでWebアプリケーションを作る方法

情報システム設計のWebアプリケーション開発課題では、プログラム言語としてJavaを利用します。

JavaでWebアプリケーションを開発する方法は色々ありますが、最も古典的な方法として、「サーブレット」と呼ばれるクラスを定義して、それを「サーブレットコンテナ」に読み込ませて実行するという方法です。
この種のWebアプリケーションの作り方は、Java Platform, Standard Editionの拡張機能・Jakarta EEにおいて「[Jakarta Servlet](https://jakartaee.github.io/servlet/)」という仕様として定められています。
この仕様に沿ってプログラムを用意すれば、JavaでWebアプリケーションを実現することができるということです。
...
HelloServlet.javaには`HelloServlet`というサーブレットのクラスが含まれています。
サーブレットコンテナを起動してサーブレットをを読み込ませるために、[サンプルコードのアプリケーションを起動](../../README.md#️-開発したアプリケーションを起動する)してください。

サーブレットコンテナが起動したら、ブラウザで http://localhost:8080/systemdesign2024/hello-servlet にアクセスしてください。
すると、

1. ブラウザは上記のURLにHTTPリクエストを送信します。
2. HTTPリクエストを受け取ったサーブレットコンテナは、そのURLに紐付けられている `HelloServlet.doGet`メソッドの内容を呼び出します。
3. `HelloServlet.doGet`メソッドには、HTTPレスポンスとしてHTML文書を返すプログラムが書いてありますので、サーブレットコンテナがレスポンスを返し終わったら、ブラウザの画面にはそのHTML文書の内容が表示されるはずです。
...
少なくとも、サーブレットの中にHTML文書の内容が文字列として直接記述されているのは、どうにかしたいです。
そこで、JSP（[Jakarta Server Pages](https://jakarta.ee/ja/specifications/pages/3.0/)）を使います。
JSPは、`src/main/webapp/WEB-INF`配下に配置されるファイルで、あるリクエストを受け取ったときに返したいHTML文書の内容を、HTML風の文法で記述することができるファイルです。
JSPファイルを用意しておき、サーブレットについては、HTML文書を返す代わりに、JSPにHTML文書を返すように指定する（**フォワード**する）ように書き替えることで、HTML文書の内容をサーブレットから取り除くことができます。

これを実際に行った例が[HelloServlet2.java](../../src/main/java/servlet/hello/HelloServlet2.java)です。
また、HelloServlet2のフォワード先のJSPは[hello-servlet.jsp](../../src/main/webapp/WEB-INF/hello/hello-servlet.jsp)です。

また、stuinfoでは、複雑なアプリケーションを整理して捉えるためのアーキテクチャを導入しています。
...
HTTPのリクエストにはメソッドと呼ばれる情報が含まれています。
今回の課題では、GETメソッドとPOSTメソッドの2種類のメソッドを扱います。

- GETメソッド：アドレスバーにURLを入力してアクセスしたとき、HTML文書の`a`要素（いわゆるハイパーリンク）をクリックしたとき、`form`要素（いわゆるフォーム）に情報を入力して送信ボタンをクリックしたときに送信することができる。
- POSTメソッド：`form`要素（いわゆるフォーム）に情報を入力して送信ボタンをクリックしたときに送信することができる。

GETメソッドだけあれば十分であるように感じます。
なぜ2種類のメソッドがあるのでしょう？

アプリケーション上の情報を変更するようなリクエストをするときにGETメソッドを使うことを考えてみましょう。
GETメソッドは`a`要素（いわゆるハイパーリンク）をクリックするなどだけで送れてしまうので、このような用途には不向きです。
POSTメソッドは`form`要素からでないと送れないので、このような用途で使われます。

以下では、そこで、メソッドの使い分けやそれを踏まえたリクエストの処理方法について記しておきます。
