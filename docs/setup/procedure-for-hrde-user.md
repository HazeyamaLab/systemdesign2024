...
1. [櫨研ユーザープロバイダーのユーザー作成画面](https://onyx.u-gakugei.ac.jp/hazelab-uap/apps/coder/users/new)にアクセスします。
2. 表示された画面に次の情報を入力します。
    - `クーポン番号`: 教室で案内しているものを入力してください。
    - `ユーザー名`: ご自身のポータルID（例: `e225499a`）
    - `メールアドレス`: ご自身の大学メールアドレス（例: `e225499a@st.u-gakugei.ac.jp`）
    - `パスワード`: アルファベット大文字、アルファベット小文字、数字をすべて組み合わせて12字以上のパスワードを作成してください。**安易に推測されるようなパスワードを使わないでください。パスワードを知っている人は作業内容を消去することもできます。**
3. 入力が終わったら`作成`ボタンをクリックします。

### 3. 櫨研リモート開発環境にログインして開発コンテナーを作成する

1. [櫨研リモート開発環境](https://emerald.u-gakugei.ac.jp/)にログインします。
    - `Email`: 2節の手順2で入力した`メールアドレス`
    - `パスワード`: 2節の手順2で入力した`パスワード`
...
4. 開発コンテナーの作成が完了するまで約1分かかります。開発コンテナーの作成に成功すると、画面中央に緑色の枠が表示されます。
...
2. 必要な拡張機能をインストールします。
    ...
    2. 画面左側の`ワークスペースの推奨事項`の右上`ワークスペースのおすすめの拡張機能をインストール`をクリックします。
