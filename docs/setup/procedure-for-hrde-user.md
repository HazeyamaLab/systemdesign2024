...
3. 開発コンテナーを作成します。
    ...
    2. 表示された画面に次の情報を入力します。
        ...
        - `Docker レジストリのホスト名`: 今回は`emerald.u-gakugei.ac.jp:5000`と入力します。
...
> [!NOTE]
> 「4. リモート開発環境上に作成した開発コンテナーに接続します」の手順で`Could not resolve hostname …`や`存在しないパイプに書き込もうとしました`などのエラーが表示されて接続に失敗した場合は、以下の作業を実施してください。
> 
> 1. 接続に失敗したウインドウを閉じます。
> 2. ブラウザのCoderの画面で開発コンテナーを開きます。
> 3. `Show parent apps`ボタンをクリックすると、もう一つ`VS Code Desktop`ボタンが表示されます。それをクリックしてVSCodeを起動します。
> 4. VSCodeのウインドウに`'Coder' 拡張機能がこの URI を開くことを許可しますか?`と表示されることがあります。`開く`をクリックして許可してください。
> 5. `Select the platform of the remote host …`と表示された場合は、`Linux`と入力して`Enter`キーを押してください。
> 6. しばらくすると、VSCodeのウインドウ左下のリモートボタンに`Coder: systemdesign2024/main`と表示されます。表示されたのを確認したら、メニューバーの`File`（`ファイル`）→`Close Remote Connection`（`リモート接続を閉じる`）をクリックします。その後、VSCodeのウィンドウを閉じます。
> 7. もう一度「4. リモート開発環境上に作成した開発コンテナーに接続します」の手順を実施してください。
...
具体的には、まず、[櫨研リモート開発環境にログイン](#3-櫨研リモート開発環境にログインして開発コンテナーを作成する)してください。ログインしたら、[作成した開発コンテナーにVSCodeを接続する](#4-作成した開発コンテナーにvscodeを接続しvscodeの設定を行う)の手順3以降と同じ手順で、開発コンテナーにVSCodeを接続してください。
